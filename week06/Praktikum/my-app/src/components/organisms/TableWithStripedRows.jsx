...
const TABLE_HEAD = ["NPM", "Name", "Prodi", "Fakultas", "alamat", "minat", "matakuliah"];
    ...
    return (
        <Card className="h-full w-full overflow-auto p-6">
            ...
            <table className="w-full min-w-max table-auto text-left">
                ...
                <tbody>
                    {users.map((user) => (
                        <tr key={user.id} className="even:bg-blue-gray-50/50 align-top">
                            <td className="p-4">{user.npm}</td>
                            <td className="p-4">{user.nama}</td>
                            <td className="p-4">{user.prodi}</td>
                            <td className="p-4">{user.fakultas}</td>
                            <td className="p-4">{user.alamat.jalan}, {user.alamat.kelurahan}. {user.alamat.kota}</td>       
                            <td className="p-4">
